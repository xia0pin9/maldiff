...
func MetaCheck(benDB, malDB *leveldb.DB, bmd5, mmd5 string) bool {
    ...
    var btopPkgs, mtopPkgs map[string]struct{}
...
func GetPairs(benSize, malSize *map[string]float64, benHashs, malHashs *map[string][]byte, keysChan chan *KeyPair) {
    ...
    for bmd5, bsize := range *benSize {
        for mmd5, msize := range *malSize {
            ...
            if threshold < bsize/msize && bsize/msize < 1/threshold {
                bhash = (*benHashs)[bmd5]
                mhash = (*malHashs)[mmd5]
                keysChan <- &KeyPair{&bhash, &mhash, bmd5, mmd5, bsize, msize}
            }
        }
    }
}
...
func main() {
    ...
    var bKey, bValue, mKey, mValue           []byte
    var bsize, msize            float64

    malHashs := make(map[string][]byte)
    malSizes := make(map[string]float64)
    maliter := malDB.NewIterator(util.BytesPrefix([]byte("h-")), nil)
    for maliter.Next() {
        mKey = maliter.Key()
        mValue = maliter.Value()
        mKeySplit = strings.Split(string(mKey[:]), "-")
        msize, _ = strconv.ParseFloat(mKeySplit[2], 64)
        malSizes[mKeySplit[3]] = msize 
        malHashs[mKeySplit[3]] = mValue
    }
    maliter.Release()

    // 16 buckets in total for each set
    for i := 1; i <= 16; i ++ {
        benHashs := make(map[string][]byte)
        benSizes := make(map[string]float64)
        ...
        for beniter.Next() {
            ...
            benSizes[bKeySplit[3]] = bsize
            benHashs[bKeySplit[3]] = bValue
        }
        beniter.Release()
        ...
        for i := 0; i < runtime.NumCPU()-4; i++ {
            wg.Add(1)
            go Worker(benDB, malDB, keysChan, resChan, wg)
        }

        go GetPairs(&benSizes, &malSizes, &benHashs, &malHashs, keysChan)
