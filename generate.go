...
func GetCode(output io.ReadCloser) *[][]string {
    var curCodes, codelist, lineSplit []string
    var line, typeStr, opcode, operand, operandOri, value string
    var matched bool

    codeIndex := make([][]string, 0)
    ...
    for scanner.Scan() {
        ...
        if starts(line, "Method(") {
            ...
            matched = false
            if len(curCodes) > 0 {
                codeIndex = append(codeIndex, curCodes)
...
func GenerateHash(codeList *[][]string) (int, *[]string, *[]byte, *Fpmap) {
    ...
    fpmap := make(Fpmap, 0)
    topPkgs := make([]string, 0)

    for _, bytecodes = range *codeList {
        fname = bytecodes[0]
        ...
        if ngsize < uint(len(bytecodes)) {
            for k = 0; k < uint(len(bytecodes)) - ngsize + 1; k++ {
                feature = strings.Join(bytecodes[k:k + ngsize], "")
                //fmt.Println(feature)
                index = djb2([]byte(feature))
                fpmap = append(fpmap, Index{index, uint(fOffset), k})
        ...
        } else if ngsize == uint(len(bytecodes)) {
...
func init() {
    ...
    flag.UintVar(&ngsize, "ngsize", 5, "N-gram size")
